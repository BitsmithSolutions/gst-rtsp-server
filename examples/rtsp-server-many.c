...
remove_sessions (GstRTSPServer * server)
{
  ...
  gst_rtsp_session_pool_filter (pool,
                (GstRTSPSessionPoolFilterFunc) remove_func,
                server);
  g_object_unref (pool);
...
static void
removed_stream (GstRTSPMedia * media, GstRTSPStream * rtspStream,
        gpointer data)
{
           ...
           gpointer data)
{
  ...
  g_signal_connect (media, "removed-stream", (GCallback) removed_stream,
            data);
...
gio_read_socket (GIOChannel * channel, GIOCondition condition, gpointer data)
{
  ...
  if (g_strrstr (buf, "\"pause\""))
    ...
    gst_rtsp_media_set_pipeline_state (rtspChannel->media,
                       GST_STATE_PAUSED);
    }
...
main (int argc, char *argv[])
{
  ...
  g_object_set (serverContext->server, "service",
        serverContext->rtspListenPort, NULL);
  ...
  for (int channelIndex = 0; channelIndex < NUM_OF_CHANNELS; channelIndex++)
    ...
    gst_rtsp_media_factory_new ();
      ...
      g_sprintf (serverContext->channel[channelIndex]->pipelineLaunch,
         pipelineLaunchStr,
         serverContext->channel[channelIndex]->listenPort);

      gst_rtsp_media_factory_set_launch (serverContext->
                     channel[channelIndex]->factoryStream,
                     serverContext->
                     channel[channelIndex]->
                     pipelineLaunch);
      gst_rtsp_media_factory_set_shared (serverContext->
                     channel[channelIndex]->factoryStream,
                     TRUE);
    ...
    gst_rtsp_address_pool_new ();
      gst_rtsp_media_factory_set_address_pool (serverContext->
                           channel[channelIndex]->
                           factoryStream,
                           serverContext->
                           channel[channelIndex]->pool);
      gst_rtsp_media_factory_set_protocols (serverContext->
                        channel[channelIndex]->
                        factoryStream,
      ...
      gst_rtsp_mount_points_add_factory (serverContext->mounts,
                     serverContext->
                     channel[channelIndex]->mountPath,
                     serverContext->
                     channel[channelIndex]->
                     factoryStream);
